...
const String kBaseUrl = "http://192.168.1.12:3000/api";
...
void setup() {
  ...
  display.Initialize();
  if (!downloadImage("black") || !downloadImage("red")) {
    return;
  }
  display.Update();
...
bool downloadImage(const String color) {
  Serial.printf("Downloading image: %s\n", color);
  HTTPClient http;

  // Request the current image from the server.
  if (!network.HttpGet(&http, kEpdEndpoint + "?color=" + color)) {
    return false;
  }

  if (color == "red") {
    display.SetRedChanel();
  }
  ...
  do {
    ...
    display.Load(buffer, count, color == "red");
